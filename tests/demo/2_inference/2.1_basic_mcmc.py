...
# # ht.set_planet_transmission_string(np.array([rs[0]]))
# # ax1.errorbar(times, observed_fluxes - ht.get_transit_light_curve(),
# #              yerr=y_sigma, fmt=".k", capsize=0)
...
if __name__ == '__main__':
    ...
    ax2.scatter(times, ht.get_transit_light_curve() - observed_fluxes,
                color='#000000', s=2)
    ...
    for sample in chain[np.random.randint(len(chain), size=50)]:
        ...
        ax1.plot(transmission_string * np.cos(theta),
                 transmission_string * np.sin(theta), color='C0', alpha=0.1)
    ...
    transmission_string = ht.get_planet_transmission_string(theta)
    ax1.plot(transmission_string * np.cos(theta),
             transmission_string * np.sin(theta), lw=1.5, c='#000000')
    ax2.plot(theta * 180. / np.pi, transmission_string,
             c='#000000', lw=1.5, label='True transmission string')
    ax1.plot(rs[0] * np.cos(theta), rs[0] * np.sin(theta),
             lw=1.5, c='#d5d6d2', ls='--')
